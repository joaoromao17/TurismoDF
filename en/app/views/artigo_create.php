<?php 
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $query = "INSERT INTO `turismodf`.`artigos` 
                (`titulo`, `texto`, `link`, `status`, `imagem`, `imagem_externa`, `categoria_id`, `usuario_id`)
                VALUES (:titulo, :texto, :link, :status, :imagem, :imagem_externa, :categoria_id, :usuario_id)";
    ...
    $query = "SELECT * FROM `turismodf`.`categorias` ORDER BY nome";
    $stmt = $dbh->prepare($query);
        ...
        <div class="main_opc">
            ...
            <section class="novo__form__section">
                ...
                <form action="" method="post" class="novo__form" enctype="multipart/form-data" >
                    ...
                    <br><br>
                    <div>
                        <label for="nome">Link</label><br>
                        <textarea name="link"cols="30" rows="15" placeholder="Informe o link do artigo" required></textarea>
                    </div>
                    <br><br>
                    <div>
